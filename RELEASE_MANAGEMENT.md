...
- **Committing changes** with subjects that read as English imperatives, and **titling pull requests** for the people who will read them in a release. Both are published verbatim — see [CONTRIBUTING.md](CONTRIBUTING.md#commit-messages) and [§6a](#6a-what-a-release-looks-like).
...
- **Generating release notes** — from the commit range between the previous tag and this one, grouped into sections, credited to their contributors, and structured as [§6a](#6a-what-a-release-looks-like) describes. Pull request labels and contributor accounts are read here, because they are the two things only the forge knows.
...
## 6a. What a release looks like

GitHub Releases are generated in a fixed structure. **Empty sections are omitted**, so most releases show three or four of these:

```markdown
# v0.2.0

## 🚀 Highlights          ← from RELEASES.yaml (see below)
## ⚠️ Breaking Changes    ← first, because it can ruin an afternoon
## 🔒 Security
## ✨ New Features
## 🔄 Improvements
## 🐛 Bug Fixes
## 📚 Documentation
## 🏗 Internal            ← CI, refactors, tests, dependencies
## 📊 Release Statistics  ← commits, contributors, files, +/- lines
## 🙌 Contributors        ← @-mentions where GitHub knows the account
## 🔗 Full Changelog      ← previous tag ... this tag
```

Commit hashes are **not** the content. The raw commit list is folded into a `<details>` block for the reader who went looking; the bullets above it are titles.

### Where the words come from

**Nothing is paraphrased, and nothing is generated by a language model.** Every line in a release was typed by a person.

| Part of the release | Written by | Where |
|---|---|---|
| Bullet text | You | The **pull request title**, or the commit subject |
| Highlights paragraph | You | `summary:` in [`RELEASES.yaml`](RELEASES.yaml) |
| Highlight bullets | You | `highlights:` in `RELEASES.yaml` |
| Section, statistics, contributors | The tool | Derived from git and the GitHub API |

This is how Terraform, Kubernetes, and GitHub CLI produce readable notes: the tooling collects and groups, and a human wrote each line. A release note is a record. An invented sentence in one is worse than a terse one, so the tool will not write it — the same rule the commit classifier follows.

When a release has no `summary:` in the roadmap, the Highlights paragraph falls back to a statement of measurements — *"4 commits from 1 contributor, across 4 files."* Honest, and a prompt to go and write something better.

### How a change lands in a section

The most reliable signal available wins:

1. **A breaking-change marker** — `feat!:`, a `BREAKING CHANGE:` footer, or a `breaking-change` label. Outranks everything.
2. **A pull request label** — `feature`, `enhancement`, `bug`, `documentation`, `ci`, `build`, `dependencies`, `refactor`, `performance`, `security`, `tests`.
3. **A Conventional Commit type** — `feat:`, `fix:`, `docs:`, `refactor:` …
4. **The files touched** — a change confined to `docs/`, `examples/`, or a root `*.md` is Documentation; one confined to `.github/`, `internal/`, `_test.go`, or `go.mod` is Internal.
5. **The leading imperative verb** — as [CONTRIBUTING.md](CONTRIBUTING.md#commit-messages) describes.

Anything unrecognised lands in **Improvements**, the section that asserts least.

Only step 2 needs the network. Without `GITHUB_TOKEN` the chain starts at step 3, which is why `make notes` works on a laptop.

Preview the notes for the next release at any time, writing nothing:
...
flowchart TB
    ...
    WF --> CL["internal/changelog<br/>CHANGELOG.md, inside the tag"]
    WF --> RN["internal/releasenotes<br/>the GitHub Release body"]
    WF --> RM["internal/roadmap<br/>RELEASES.yaml, the Highlights"]
